...
num$num <- round(maxMHchip * (num$num / sum(num$num)))
# account for any rounding error
num$num[which.max(num$num)] <- num$num[which.max(num$num)] + maxMHchip - sum(num$num)
...
# select extra SNPs to make up the HD panel
extraLineNumbers <- 1:chosenLoci[[3]] # this is 1:total number of SNPs
extraLineNumbers <- extraLineNumbers[!(extraLineNumbers %in% c(chosenLoci[[1]]$lineNumber, mhLineNumbers))]
# sampling line numbers that are NOT qtl or in mh panel to make up HD panel
extraLineNumbers <- sample(extraLineNumbers, HDpanelSize - length(mhLineNumbers), replace = FALSE)

# read genotypes from chosen loci in from VCF
# specify by line number for speed
inputGenos <- vcf_readLoci(vcfPath = inputVCFpath, 
													 lineNumbers = c(chosenLoci[[1]]$lineNumber, mhLineNumbers, extraLineNumbers),
													 numLines = 20000)
...
for(i in 1:nrow(num)){
	...
	snpPos[[i]] <- which(!(inputGenos[[2]]$pos[tempBool] %in% chosenLoci[[1]]$pos[chosenLoci[[1]]$chr == num$chr[i]]))
	snpChipPositions[[i]] <- inputGenos[[2]][tempBool,][snpPos[[i]],] %>% 
...
numLoci <- c(50, 100, 150, 200, 250, 350, maxMHchip)
numLoci <- numLoci[numLoci <= maxMHchip]
...
# now we choose the highest scoring from each chromosome
allPanels <- list()
for(i in 1:length(tempNum)){ # for each panel
	allPanels[[i]] <- data.frame()
	for(j in 1:nrow(tempNum[[i]])){ # for each chr
		tempLoci <- snpChipPositions[[j]]
		tempChosen <- chosenLoci[[2]][chosenLoci[[2]]$chr == tempLoci$chr[1],] %>%
			arrange(desc(score)) %>% slice(1:tempNum[[i]]$num[j])
		tempChosen <- unlist(str_split(tempChosen$posConcat, ","))
		allPanels[[i]] <- rbind(allPanels[[i]], tempLoci %>% filter(pos %in% tempChosen) %>% select(name) %>% 
			rename(id = name) %>% left_join(snpMap, by = "id"))
	}
}
allPanels[[length(allPanels) + 1]] <- snpMap
numLoci <- c(numLoci, nrow(snpMap))
